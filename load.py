...
class Preproc:
    ...
    def process_x(self, x):#??? pad normalization???
        x = pad(x)
...
def pad(x, val=0, dtype=np.float32):# padding for all ecg with same length = max(len(x))/ label with 3: others
...
def load_dataset(data_json):
    ...
    for d in tqdm.tqdm(data):
        ...
        ecgs.append(load_ecg(d['ecg']))## load .mat file
    return ecgs, labels
